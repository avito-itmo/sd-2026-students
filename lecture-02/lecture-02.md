...
### Паттерны сложных взаимодействий: Transactional Outbox, Saga, CQRS, Event Sourcing

Эти паттерны помогают управлять сложными потоками данных и обеспечить согласованность в масштабируемых системах.

#### Transactional Outbox

Разберем пример кода, который сохраняет новый заказ в БД и затем публикует сообщение об этом в брокер.
...
// Публикуем сообщение о заказе в брокер
broker.SendMessage(new Message(order));
```

Как говорилось на первой лекции, достичь 100% доступности системы невозможно. Если доступность брокера гарантируется на уровне 99,9%, то на длительном промежутке времени в системе может терятся каждое 1000-е сообщение. Технически здесь проблема в отсутствии общей транзакции между БД и брокером.

Типовое решение этой проблемы - паттерн Transactional Outbox. Его идея заключается в том, что бы в БД завести таблицу исходящих сообщений (Outbox) и вместо отправки сообщения в брокер, сохранять исходящее сообщение в таблицу Outbox в рамках той же транзакции, в которой создается Заказ. Общая транзакция будет гарантировать, что Заказ и исходящее сообщение всегда будут создаваться вместе.

``` csharp
// Сохраняем заказ в БД
dbContext.Orders.Add(order);
dbContext.Outbox.Add(new Message(order));

dbContext.SaveChanges();
transaction.Commit();
```

Разбором таблицы Outbox занимается фоновый воркер, который в цикле проверяет наличие новых сообщений в таблице Outbox. При появлении новых сообщений, эти сообщения публикуются в брокер, после чего удаляются из БД.
Здесь нужно понимать, что у БД досутпность тоже не равна 100%, и из-за недоступности БД могут происходить ошибки при удалении сообщений после отправки в брокер, что будет приводить к повторной отправке уже отправленных сообщений. Это нормально. Для решения этой проблемы на стороне потребителя должна быть реализова дедупликация входящих сообщений.

``` csharp
while (true) {
    var messages = dbContext.Outbox.GetAll();
    var success = new List<Message>();
    foreach(var message in messages) {
        var ok = broker.SendMessage(message);
        if (ok) {
            success.Add(message);
        }
    }

    dbContext.Outbox.RemoveRange(success);
}
```
